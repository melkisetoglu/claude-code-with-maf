...
- `FileMemoryProvider` is the **third of five** MAF context-provider types this workshop ends up covering from the framework's catalog: Compaction (Step 10), Skills (Step 11), Memory (Step 12), Todos (Step 13), SubAgents (Step 15). All attach via `options.AIContextProviders`, all persist state through `StateKeys` (when they have any), and **most of them auto-register tools** (Skills → `load_skill`; Memory → 5 `FileMemory_*`; Todos → 5 `TodoList_*`; SubAgents → 6 `SubAgents_*`). The pattern is solid enough by now that **Step 13 (`TodoProvider`)** will follow the same recipe — probe its tool list first.
