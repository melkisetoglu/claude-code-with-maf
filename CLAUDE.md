...
Current state: **Step 16 — MCP server integration via the standalone SDK + dedicated approval middleware**. agent.json gains an `mcpServers` field; at startup, for each entry the agent uses the `ModelContextProtocol` 1.3.0 SDK to open `HttpClientTransport`, create `McpClient`, call `ListToolsAsync()` (the tools are `McpClientTool : AIFunction` so they go straight into the agent's tool list — no wrapping needed). Approval is handled by a **dedicated function-invocation middleware** (`McpApprovalMiddleware`) wired in the `AIAgentBuilder` `.Use` chain alongside Step 14's tool-timing middleware. Slash command `/mcp` lists configured servers (headers redacted). Fail-fast at startup with a clear error message. **Three pivots shape this step:** (1) `HostedMcpServerTool` doesn't reach the model in the preview Anthropic adapter (silent translation gap; verified live). (2) Pivoted to the standalone SDK — tools discoverable, model sees them. (3) Wrapping `McpClientTool` with `ApprovalRequiredAIFunction` silently broke invocation — after approval the framework didn't actually call the tool (verified live: model looped, `claudechat.log` showed only MCP ping keepalives, no `tools/call`); pivoted again to function-invocation-seam approval middleware that controls `next(fnCtx, ct)` directly. `ApprovalState` moved from `ChatLoop` to `Program.cs` so `AgentBuilder` can capture it for the middleware. Same `[y/N/a=always]` UX, different mechanism. Required version bump: `Microsoft.Extensions.AI[.Abstractions]` 10.5.1 → 10.5.2 (SDK's minimum). Workshop doesn't ship a hardcoded `agent.json` (opt-in by user creation). Verified end-to-end with Playwright MCP (`npx @playwright/mcp@latest --port 8931`) — 26 browser tools visible, approval gate fires per call, tools actually execute (`→ 5051ms` for real `browser_navigate`). Tests: 9 new (5 McpConfigTests + 4 McpCommandTests); 169 unit tests total. Workshop-level rule: when the framework's approval flow appears to swallow a tool call, check whether the wrapped function is a custom `AIFunction` subclass — if so, bypass `ApprovalRequiredAIFunction` and gate at the function-invocation middleware seam instead.
