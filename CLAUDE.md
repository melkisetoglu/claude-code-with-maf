...
Current state: **Step 9 — streaming polish**. Three additions: Ctrl+C cancels the in-flight turn (per-turn `CancellationTokenSource` hooked to `Console.CancelKeyPress`, `e.Cancel = true` keeps the process alive); a Braille spinner runs in a background `Task` between `claude > ` and the first streamed content (auto-disabled when stdout is redirected via `Console.IsOutputRedirected`); a `MarkdownStreamRenderer` toggles dim-cyan ANSI colour around triple-backtick fences (also auto-disabled when redirected; state tracked even when colours off). ANSI escape constants built at field-init time via `(char)27 + "[2;36m"` because raw ESC bytes get stripped by some tools.
