...
Current state: **Step 14 — fluent middleware pipeline via `AIAgentBuilder`**. The wrap chain in `AgentBuilder.cs` switched from imperative `inner = new XAgent(inner)` reassignment to the framework's fluent builder: `new AIAgentBuilder(inner).UseLogging(loggerFactory, _ => { }).UseOpenTelemetry(name, _ => { }).UseToolApproval(jsonOpts).Use(ToolTimingMiddleware).Build(serviceProvider)`. Same behavior; cleaner shape. The empty `ServiceCollection().BuildServiceProvider()` is load-bearing (the .Use chain dereferences it internally) and is the workshop's first DI hook — future steps will grow it. **New piece of original middleware**: `ToolTimingMiddleware` via `FunctionInvocationDelegatingAgentBuilderExtensions.Use(...)` — prints `→ Nms` on its own line after every tool call, including the framework's auto-registered `load_skill` / `FileMemory_*` / `TodoList_*`. Sits BELOW the approval gate so denied calls don't get fake-timed. The retroactive lesson — fourth in the chain after Steps 11/12/13: the framework already shipped what we hand-rolled (the wrap chain itself). Step 3's `ToolApprovalAgent` and Step 5's `LoggingAgent` / `OpenTelemetryAgent` could have been one-line `.UseX()` extension calls all along. No new unit tests this step (refactor is behavior-preserving; middleware needs a real `FunctionInvocationContext`, ceremony exceeds coverage); live smoke confirms timing visible on every tool call. 153 tests still pass. A `[doc]` commit after this lands will add retro-callouts to Steps 3 and 5 chapters pointing at the builder extension methods.
